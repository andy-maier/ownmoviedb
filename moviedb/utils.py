...
def test_ParseComplexTitle():
    ...
    _num_failed += test_one_ParseComplexTitle(
        "A",
        {"title": "A",
         "year": None,
         "series_title": None,
...
def test_one_ParseComplexTitle(complex_title, exp_rv):
    ...
    if act_rv["title"] != exp_rv["title"]:
        failure_txt += "\nActual title %s does not match expected title %s" % \
            (repr(act_rv["title"]), repr(exp_rv["title"]))
    if act_rv["year"] != exp_rv["year"]:
        failure_txt += "\nActual year %s does not match expected year %s" % \
            (repr(act_rv["year"]), repr(exp_rv["year"]))
    if act_rv["series_title"] != exp_rv["series_title"]:
        failure_txt += "\nActual series_title %s does not match expected " \
            "series_title %s" % \
            (repr(act_rv["series_title"]), repr(exp_rv["series_title"]))
    if act_rv["episode_id"] != exp_rv["episode_id"]:
        failure_txt += "\nActual episode_id %s does not match expected " \
            "episode_id %s" % \
            (repr(act_rv["episode_id"]), repr(exp_rv["episode_id"]))
    if act_rv["episode_title"] != exp_rv["episode_title"]:
        failure_txt += "\nActual episode_title %s does not match expected " \
            "episode_title %s" % \
            (repr(act_rv["episode_title"]), repr(exp_rv["episode_title"]))
    if failure_txt != "":
        failure_txt = ("\nTest failure: ParseComplexTitle(%s):" %
                       repr(complex_title)) + \
            failure_txt
        print failure_txt
        sys.stdout.flush()
